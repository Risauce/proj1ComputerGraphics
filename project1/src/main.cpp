...
//---------------------------------------------------
//This program is an introduction to using textures in OpenGL graphics.
//---------------------------------------------------
...
void processInput(Matrix4& model, GLFWwindow* window, int& mode) {
    if (isPressed(window, GLFW_KEY_ESCAPE) || isPressed(window, GLFW_KEY_Q)) {
        glfwSetWindowShouldClose(window, true);
    }
    else if (isSpaceEvent(window)) {
        
        mode = (mode + 1)%2; //This updates the mode between 0 and 1
    }
...
GLuint createTexture() {
    ...
    GLuint texCoords[WIDTH * HEIGHT]; //62,500

    for (int row = 0; row < WIDTH; row++)
    {
        for (int col = 0; col < HEIGHT; col++)
        {
            unsigned int currentPix = row * WIDTH + col;
            if ((row % 50 < 25 && col % 50 < 25) || (row % 50 > 25 && col % 50 > 25))  //White is First now. 
            {
                texCoords[currentPix] = 0xFFFFFFFF; //White
            }
            else if ((row % 50 > 25 && col % 50 < 25) || (row % 50 < 25 && col % 50 > 25))
            {
                texCoords[currentPix] = 0x00000000; //This is black.
            }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, WIDTH, HEIGHT, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, &texCoords);
...
GLuint loadTexture(const std::string& path, bool flip = true) {
    ...
    if (data) {
        ...
        //**
         //* TODO: Part-3 create a texture map for an image         DONE
...
int main(void) {
    ...
    // setup the textures
    //** TODO: Part2 create and bind the texture. DONE
    ...
    GLuint textureQuestionMark = loadTexture("img/question.png");
    glBindTexture(GL_TEXTURE_2D, textureQuestionMark); 
    
    //The image checker (doesn't work very well)
    glActiveTexture(GL_TEXTURE2);
    GLuint textureAnything = loadTexture("img/giraffe.png");
    glBindTexture(GL_TEXTURE_2D, textureAnything);


    //glActiveTexture(GL_TEXTURE3);
    //GLuint textureAnythingElse = loadTexture("img/hazardstripes.png");
    //glBindTexture(GL_TEXTURE_2D, textureAnythingElse);
    ...
    while (!glfwWindowShouldClose(window)) {
        // process input
        processInput(model, window, mode);
        ...
        //Change which texture to send the shaders on space (mode):
        Uniform::set(shader.id(), "ourTexture", mode);
